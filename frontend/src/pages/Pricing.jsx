          ...
          {PLANS.map((plan) => (
            <div
              ...
              className={`rounded-2xl p-8 border transition hover:-translate-y-1 ${
                plan.highlight
                  ? 'border-border bg-signal text-white shadow-xl hover:shadow-2xl'
                  : 'border-border bg-surface hover:shadow-md dark:hover:shadow-none hover:border-border dark:hover:border-border'
              }`}
            >
              <h2 className={`text-lg font-semibold mb-1 ${plan.highlight ? 'text-white' : 'text-foreground dark:text-white'}`}>
                {plan.name}
              </h2>
              <p className={`text-sm mb-6 ${plan.highlight ? 'text-muted-foreground' : 'text-muted-foreground'}`}>
                {plan.description}
              </p>
              <div className="flex items-baseline gap-1.5 mb-6">
                <span className="text-4xl font-extrabold">{plan.price}</span>
                <span className={plan.highlight ? 'text-muted-foreground' : 'text-muted-foreground'}>{plan.cadence}</span>
              </div>
              ...
              <ul className="space-y-3 mb-8">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-sm">
                    ...
                    <span className={plan.highlight ? 'text-muted-foreground' : 'text-muted-foreground'}>{f}</span>
                  </li>
                ))}
              </ul>
